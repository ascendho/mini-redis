...
/*
 * 渐进式哈希是哈希表在扩容（或缩容）时采用的一种优化策略：
 * 它不一次性将旧哈希表中的所有节点迁移到新哈希表，而是将迁移过程分散到多次操作（如插入、查询、删除）中，
 * 每次迁移少量节点，逐步完成整个哈希表的更新。
 *
 * 渐进式哈希的三大优势：
 * 1. 避免性能波动
 *    渐进式哈希通过“少量多次”的迁移策略，将总迁移成本分散到多次普通操作中，
 *    每次迁移仅消耗极短时间（如代码中每次最多128个节点），保证单次操作的响应速度稳定。
 *
 * 2. 保证操作可用性
 *    迁移过程中，哈希表仍能正常处理读写操作：
 *    - 新插入的节点直接放入 newer 表；
 *    - 查询/删除操作会同时检查 newer 和 older 表（如 hm_lookup 先查新表再查旧表）；
 *    整个过程用户无感知，不会因“正在扩容”而导致服务不可用。
 *
 * 3. 平衡资源开销
 *    一次性迁移需要临时占用大量内存（同时保留新旧表）和计算资源（重新计算所有节点的索引）。
 *    渐进式哈希每次迁移少量节点，资源占用更平缓，避免短时间内的资源峰值。
 *
 * 总结：渐进式哈希是哈希表扩容时的 “平滑过渡方案”，
 *      核心价值是在保证功能正确性的前提下，避免一次性重哈希带来的性能抖动，确保系统稳定性和响应速度。
 *
 */
